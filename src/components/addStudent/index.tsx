...
const AddStudent = ({
  ...
  const [seniorSuggestions, setSeniorSuggestions] = useState(['John Doe', 'Jane Doe', 'James Doe']); // -> sample senior names
  const seniorRef = useRef<HTMLInputElement>(null);
  // const [errors, setErrors] = useState({}); -> update if needed

  // const validate = (data) = {} -> update if needed
  // const getSeniors = async () => {} -> update when established
  ...
  const handleSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    // setErrors(validate(studentData)); -> update if needed

    // -> update when endpoint is finished
    //
    ...
    console.log(studentData); // -> for now
  }
    ...
    <>
      <div className='bg-[#F5F5F5] w-[84%] h-screen absolute inset-y-0 right-0 font-sans'>
        ...
        <div className='flex flex-row justify-between items-center m-auto w-[96%] h-[14%]'>
          <span className='text-4xl font-medium text-[#000022] leading-[140%]'>Add New Member</span>
          <div className='w-16 h-16 rounded-full bg-[#BCDFFB]'> {/* -> update when more established */}
            {icon 
              ? <Image 
                  src={icon}
        ...
        <section className='flex items-center justify-center bg-white m-auto w-[96%] rounded-xl h-[72%]'>
          <div className='flex flex-col w-[75%] h-[78%] justify-between items-center'>
            <div className='flex flex-col w-[15%] h-[26%] items-center'>
              {isPicUploaded
                ? <span>picture.jpeg</span> // -> update functionality when established
                : <button className='rounded-full relative bg-[#F5F6FA] w-[5.5rem] h-[5.5rem] flex items-center'>
              ...
              <button className='text-[14px] text-[#22555A] w-24 h-6 mt-3'>Upload Photo</button>
            </div>
              ...
              <div className='flex flex-row w-[98%] h-[75%] justify-between items-center'>
                <div className='flex flex-col w-[47%] h-full justify-between text-[#515151]'>
                  ...
                  {/* <div className='flex flex-col w-full h-[26.6%]'>
                    ...
                    <input 
                      className='w-full h-[63%] bg-[#F5F6FA] rounded border-[0.3px] border-[#A6A6A6] placeholder:text-[#A6A6A6] placeholder:font-normal placeholder:text-[14px] pl-[12px]' 
